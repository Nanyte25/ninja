	...
	public function index() {
		...
		if($content_from == 'filter') {
			...
			if($form_model->get_value('threshold_onoff') && isset($perf_data['warn'], $perf_data['crit'])) {
				if(performance_data::check_against_threshold($perf_data['crit'], $perf_data['value'])) {
					$state = 'critical';
				} else if(performance_data::check_against_threshold($perf_data['warn'], $perf_data['value'])) {
					$state = 'warning';
				} else {
					$state = 'ok';
				}
